...
export class EditorImportTitle extends React.PureComponent<Props, State> {
  ...
  _handleReimport = async () => {
    ...
    try {
      ...
      const repo = `https://${match ? match[0] : ''}`;
      ...
      if (subpath) {
        params.subpath = subpath[0].slice(1);
      }
      // Takes data of the form: [host]/[owner]/[repo]:[path]@[branch]
      // and extracts the branch
      const branch = data.match(/@([\w,\-,\_.\/]+)/);
      if (branch) {
        params.branch = branch[0].slice(1);
      }

      const res = await fetch(`${IMPORT_API_URL}?${querystring.stringify(params)}`);
        ...
        <div className={css(styles.header)}>
          ...
          <p
            ...
            <svg
              ...
              style={{ fill: status === 'error' ? 'red' : undefined }}>
              <path
                d={
                  status !== 'error'
                    ? // Circular Arrow
            ...
            <span className={status === 'error' ? css(styles.error) : ''}>{description}</span>
          </p>
        </div>
      </div>
    );
